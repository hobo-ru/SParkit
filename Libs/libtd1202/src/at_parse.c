...
void AT_Init(void)
{
	...
	if (TD_FLASH_Read(AT_persist_buffer, persist_size) == true) {
...
void AT_Parse(char c)
{
	...
	uint8_t persist_size = 1, persist_total, extension_write, *persist_pointer;
	...
	case AT_WRITE_CONFIG:
		if (AT_argc == 0) {
			...
			*persist_pointer |= (AT_banner == true) ? 0x10 : 0x00;
			...
			if (persist_size > AT_PERSIST_SIZE) {
				persist_total = AT_PERSIST_SIZE;
			} else {
				persist_total = persist_size;
			}
			for (extension = 0; extension < AT_EXTENSION_NUMBER; extension++) {
				if (AT_extension[extension] == 0) {
					break;
			...
			TD_FLASH_Write(AT_persist_buffer, persist_total);
		} else {
			result = AT_ERROR;
		}
		break;
